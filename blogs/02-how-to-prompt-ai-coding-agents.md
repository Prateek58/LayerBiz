...
### ❌ The Symptom Prompt:
> *"The frontend is showing white screen for blog post 12. Make it work."*
...
To get high-fidelity results from coding agents, structure complex requests using this three-part formula:

```
┌─────────────────────────────────────────────────────────────┐
│             The 3-Step Agentic Prompt Framework             │
├─────────────────────────────────────────────────────────────┤
│ 1. Core Intent     │ What business/technical goal to achieve│
│ 2. Constraints     │ Security, zero-cost, no breaking edits │
│ 3. Verification    │ How the agent should test & prove it   │
└─────────────────────────────────────────────────────────────┘
```
...
By specifying the **constraints** (free, no external keys) and the **verification** (run build), the agent implements pure native code instead of bundling expensive 3rd-party SaaS libraries.
...
* Instead of: *"Make it look cooler."*
* Use: *"Ensure code blocks have transparent backgrounds, subtle `#1e293b` borders, and responsive padding (`p-6 sm:p-12`) so syntax doesn't overflow mobile screens."*
