 ...
 * Reads environment variables from shell or automatically discovers .env.local / .env.production.
 * 
 * Usage:
 *   Local: node backend/scripts/seed-articles.js
 *   Prod:  STRAPI_API_URL="http://127.0.0.1:1337" STRAPI_API_TOKEN="your_token" node backend/scripts/seed-articles.js
 */
...
// Helper to load env files if not set in environment
...
if (STRAPI_TOKEN) {
  ...
  console.log(`⚠️  [LayerBiz Seeder] No STRAPI_API_TOKEN found. Public permissions required if not tokenized.`);
}
...
async function seed() {
  ...
  console.log(`📚 Found ${files.length} article files in /blogs.\n`);
  ...
  console.log(`\n🎉 [LayerBiz Seeder] All articles successfully synced to Strapi!\n`);
}
